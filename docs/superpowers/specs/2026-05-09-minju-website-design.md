...
- **托管**：境外服务器（计划 Cloudflare Pages + R2）+ 境外域名
- **预期生命周期**：5-10 年
...
1. **域名**：建议 `.org` 后缀强化非营利定位（如 `minju-archive.org`、`xiqu-archive.org`）。备选：`.cc`、`.dev`。
2. **LLM 调用预算**：Claude API 总花费预估 $10-30，需用户确认是否接受。
3. **封面图来源**：从视频抽帧 / 网络搜集 / 用户提供？V1 可以接受占位图。
4. **首页"精选 8-12 部"如何选**：算法（按 production 数量）还是人工 curated？
5. **暗色模式**：V1 包含还是 V2？
6. **是否引入分析工具**：Plausible（隐私友好，$9/月）vs 自部署 Umami vs 不装。建议 V1 不装，V2 视需要再加。
